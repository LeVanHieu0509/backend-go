CREATE TABLE `sd_product` (
  ...
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'updated timestamp',
  PRIMARY KEY (`productId`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='spu';

CREATE TABLE `sku` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `sku_no` varchar(32) DEFAULT '' COMMENT 'sku_no', -- Mã SKU duy nhất
...
CREATE TABLE `spu_to_sku` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT COMMENT 'id',
  `sku_no` varchar(32) NOT NULL DEFAULT '' COMMENT 'sku id', --Mã SKU
  `spu_no` varchar(32) NOT NULL DEFAULT '' COMMENT 'spu id', --Mã SPU
  `is_deleted` tinyint(1) DEFAULT '0' COMMENT '0:deleted 1:null',
